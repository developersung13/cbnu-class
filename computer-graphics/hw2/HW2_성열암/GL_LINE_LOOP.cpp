...
void MyDisplay() {
    ...
    glBegin(GL_LINE_LOOP); // 다중선 폐합(모든 정점이 연결)으로 그리기 시작
...
int main(int argc, char** argv) { // main 함수 정의
    ...
    glutCreateWindow("GL Modeling - 2D Primitive(GL_LINE_LOOP)"); // "GL Modeling - 2D Primitive(GL_LINE_LOOP)"의 타이틀로 새로운 Window(창) 생성
