...
///*                - - - - - - -                                          */
///* 기능 : GL_TRIANGLE_STRIP를 사용하여 점을 표현                               */
///*                - - - - - - -                                          */
///*************************************************************************/
...
void MyDisplay() {
    ...
    glBegin(GL_TRIANGLE_STRIP); // 정점 3개 당 하나의 삼각형 그리기(내부 색 채움)
    glVertex3f(1.7, 0, 0); // 점(1) 좌표 표시
    glVertex3f(1.3, 1, 0); // 점(2) 좌표 표시
    glVertex3f(1.0, 0.4, 0); // 점(3) 좌표 표시
    glVertex3f(0.3, 1, 0); // 점(4) 좌표 표시
    glVertex3f(0.1, 0.1, 0); // 점(5) 좌표 표시
    glVertex3f(-0.6, 0.7, 0); // 점(6) 좌표 표시
    glVertex3f(-1, 0, 0); // 점(7) 좌표 표시
    glEnd(); // 그리기 종료
...
int main(int argc, char** argv) { // main 함수 정의
    ...
    glutCreateWindow("GL Modeling - 2D Primitive(GL_TRIANGLE_STRIP)"); // "GL Modeling - 2D Primitive(GL_TRIANGLE_STRIP)"의 타이틀로 새로운 Window(창) 생성
