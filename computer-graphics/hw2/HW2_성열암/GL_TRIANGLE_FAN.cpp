...
///*                - - - - - - -                                          */
///* 기능 : GL_TRIANGLE_FAN를 사용하여 점을 표현                                 */
///*                - - - - - - -                                          */
///*************************************************************************/
...
void MyDisplay() {
    ...
    glBegin(GL_TRIANGLE_FAN); // 앞으로 그려질 정점으로 기준으로 3개 당 하나의 삼각형을 색을 채워 그림
    glVertex3f(-1, -1, 0); // 점(1) 좌표 표시
    glVertex3f(1, -0.7, 0); // 점(2) 좌표 표시
    glVertex3f(0.8, -0.3, 0); // 점(3) 좌표 표시
    glVertex3f(0.7, 0.1, 0); // 점(4) 좌표 표시
    glVertex3f(-0.1, 0.5, 0); // 점(5) 좌표 표시
    glVertex3f(-0.7, 0.5, 0); // 점(6) 좌표 표시
    glEnd(); // 그리기 종료
...
int main(int argc, char** argv) { // main 함수 정의
    ...
    glutCreateWindow("GL Modeling - 2D Primitive(GL_TRIANGLE_FAN)"); // "GL Modeling - 2D Primitive(GL_TRIANGLE_FAN)"의 타이틀로 새로운 Window(창) 생성
