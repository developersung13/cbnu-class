...
void MyDisplay() {
    ...
    glBegin(GL_LINES); // 선(정점-정점)으로 그리기 시작
...
int main(int argc, char** argv) { // main 함수 정의
    ...
    glutCreateWindow("GL Modeling - 2D Primitive(GL_LINES)"); // "GL Modeling - 2D Primitive(GL_LINES)"의 타이틀로 새로운 Window(창) 생성
