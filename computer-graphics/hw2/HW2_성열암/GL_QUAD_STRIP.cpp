...
void MyDisplay() {
    ...
    glBegin(GL_QUAD_STRIP); // 4개씩 정점을 연결하여 사각형을 그리기(기존 GL_QUADS와는 정점 정의 순서 차이가 있음)
    glVertex3f(0.9, -1, 0); // 점(1) 좌표 표시
    glVertex3f(1.0, -0.7, 0); // 점(2) 좌표 표시
    glVertex3f(0.3, -1, 0); // 점(3) 좌표 표시
    glVertex3f(0.7, -0.4, 0); // 점(4) 좌표 표시
    glVertex3f(0.1, -0.6, 0); // 점(5) 좌표 표시
    glVertex3f(0.15, 0, 0); // 점(6) 좌표 표시
    glVertex3f(-0.5, -0.4, 0); // 점(7) 좌표 표시
    glVertex3f(-0.4, -0.1, 0); // 점(8) 좌표 표시
    glEnd(); // 그리기 종료
...
int main(int argc, char** argv) { // main 함수 정의
    ...
    glutCreateWindow("GL Modeling - 2D Primitive(GL_QUAD_STRIP)"); // "GL Modeling - 2D Primitive(GL_QUAD_STRIP)"의 타이틀로 새로운 Window(창) 생성
