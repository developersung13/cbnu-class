...
void MyDisplay() {
    ...
    glBegin(GL_LINE_STRIP); // 다중선(첫 정점->마지막 정점까지만)으로 그리기 시작
...
int main(int argc, char** argv) { // main 함수 정의
    ...
    glutCreateWindow("GL Modeling - 2D Primitive(GL_LINE_STRIP)"); // "GL Modeling - 2D Primitive(GL_LINE_STRIP)"의 타이틀로 새로운 Window(창) 생성
