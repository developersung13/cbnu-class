...
void MyDisplay() {
    ...
    glBegin(GL_TRIANGLES); // 정점 3개 당 하나의 삼각형을 그리기
    glVertex3f(-0.5, -1, 0); // 점(1) 좌표 표시
    glVertex3f(0.5, -1, 0); // 점(2) 좌표 표시
    glVertex3f(1, 0, 0); // 점(3) 좌표 표시 (첫 번째 삼각형)
    glVertex3f(0.5, 1, 0); // 점(4) 좌표 표시
    glVertex3f(-0.5, 1, 0); // 점(5) 좌표 표시
    glVertex3f(-1, 0, 0); // 점(6) 좌표 표시 (두 번째 삼각형 생성)
    glEnd(); // 그리기 종료
...
int main(int argc, char** argv) { // main 함수 정의
    ...
    glutCreateWindow("GL Modeling - 2D Primitive(GL_TRIANGLES)"); // "GL Modeling - 2D Primitive(GL_TRIANGLES)"의 타이틀로 새로운 Window(창) 생성
