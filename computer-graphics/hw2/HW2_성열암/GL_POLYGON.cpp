...
void MyDisplay() {
    ...
    glBegin(GL_POLYGON); // 정점 연결로 구성된 도형을 색을 채움(반시계 방향 순)
...
int main(int argc, char** argv) { // main 함수 정의
    ...
    glutCreateWindow("GL Modeling - 2D Primitive(GL_POLYGON)"); // "GL Modeling - 2D Primitive(GL_POLYGON)"의 타이틀로 새로운 Window(창) 생성
